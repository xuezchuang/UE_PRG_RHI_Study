...
enum GFSDK_SSAO_Status
{
    ...
    GFSDK_SSAO_D3D12_INVALID_FRAME_INDEX                    // FrameIndex is outside the supported frame-slot range.
};
...
struct GFSDK_SSAO_ShaderResourceView_D3D12
{
    ...
    // The SRV dimension must be TEXTURE2D for SampleDesc.Count == 1, and TEXTURE2DMS otherwise.
...
#define GFSDK_SSAO_MAX_FRAMES_IN_FLIGHT_D3D12 4

//---------------------------------------------------------------------------------------------------
// Note: The RenderAO, RenderAOMultiFrame, PreCreateRTs and Release entry points should not be called simultaneously from different threads.
//---------------------------------------------------------------------------------------------------
...
public:
    ...
    //    * This legacy entry point uses frame slot 0. Before calling it again on the same context, the caller must ensure
    //      that the GPU has completed the command list containing the previous RenderAO call.
    //    * Applications with multiple frames in flight should use RenderAOMultiFrame instead.
    //
    // Returns:
    //     GFSDK_SSAO_NULL_ARGUMENT                        - One of the required argument pointers is NULL
    //     GFSDK_SSAO_INVALID_PROJECTION_MATRIX            - The projection matrix is not valid
    ...
    GFSDK_SSAO_Status RenderAOMultiFrame(
        ID3D12CommandQueue* pCmdQueue,
        ID3D12GraphicsCommandList* pCmdList,
        const GFSDK_SSAO_InputData_D3D12& InputData,
        const GFSDK_SSAO_Parameters& Parameters,
...
//---------------------------------------------------------------------------------------------------
// Renders SSAO using a caller-managed frame slot.
//
// Remarks:
//    * FrameIndex must be less than GFSDK_SSAO_MAX_FRAMES_IN_FLIGHT_D3D12.
//    * The caller must not reuse a FrameIndex until the GPU has completed the command list containing
//      the previous call that used the same FrameIndex.
//    * Different frame slots use independent constant-buffer storage and do not require a per-frame GPU wait.
//    * Command lists recorded with the same context must be submitted to the command queue in call order.
//
// Returns:
//     GFSDK_SSAO_D3D12_INVALID_FRAME_INDEX - FrameIndex is not less than GFSDK_SSAO_MAX_FRAMES_IN_FLIGHT_D3D12
//     The same status values as GFSDK_SSAO_Context_D3D12::RenderAO otherwise.
//---------------------------------------------------------------------------------------------------
GFSDK_SSAO_DECL(GFSDK_SSAO_Status, GFSDK_SSAO_RenderAO_D3D12_MultiFrame,
...
inline GFSDK_SSAO_Status GFSDK_SSAO_Context_D3D12::RenderAOMultiFrame(
    ID3D12CommandQueue* pCmdQueue,
    ID3D12GraphicsCommandList* pCmdList,
    const GFSDK_SSAO_InputData_D3D12& InputData,
    const GFSDK_SSAO_Parameters& Parameters,
    ...
    GFSDK_SSAO_UINT FrameIndex)
{
    return GFSDK_SSAO_RenderAO_D3D12_MultiFrame(this, pCmdQueue, pCmdList, InputData, Parameters, Output, RenderMask, FrameIndex);
}
